...
| Feature | Status |
|---|---|
| Login (UID/Password) | Full — checked against the `Users` sheet; blocked if that user's Status is `Disabled` |
| Work Reports (view + submit) | Full — Employee Name is locked to the logged-in user, both in the UI and re-enforced server-side |
| Salary (real-time calculator + what-if calculator) | Full — employee's own Salary page computes live from Attendance, plus a "Salary Calculator" to estimate pay for any number of present days; admin gets an "Update Salary" calculator in each employee's drawer |
...
| Attendance + Holidays (combined calendar) | Full — employees see the current month, admins see all 12; shows Present/Absent/Leave/Holiday per day plus a count above the calendar |
...
| Employees directory (admin) | Full — includes Add Employee, Disable/Enable Employee, and editing Designation/Department/Employment Type |
...
| Apply Leave / Approve Leave | Full — Dashboard "Apply Leave" submits a request; admin approves/rejects per-employee from the Employees drawer |
| Request/Approve Advance Salary | Full — Dashboard "Request Advance Salary" submits a request; approving one adds the amount to that employee's Advance Taken |
...
| Settings page (Company Name / Working Days Per Month) | Full — Working Days Per Month feeds directly into every salary calculation |
...
**Settings** — Key | Value *(rows: "Company Name", "Working Days Per Month")*
