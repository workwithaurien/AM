...
| Warning/Performance Note/Appreciation letters | Full — issued from the Employees drawer. Warning and Appreciation are formal documents: Subject + Body, and once issued open a formatted, letterhead-style printable view with a "Download as PDF" button (uses the browser's native print-to-PDF — no external library). Warning letters are numbered automatically (1st/2nd/3rd) and capped at 3 per employee — issuing a 4th is blocked server-side with a clear error. Both types are listed (with a "View PDF" button to reopen the same formatted view any time) on the employee's own Profile page and in the Employees drawer's "Letters" tab. Performance Note stays a simple single-message note, no Subject/PDF |
...
| CEO role (approves Admin-submitted requests) | Full — a third `Users.Role` value, `ceo`, gets every Admin capability (Employees page, all approvals, Settings) plus the exclusive right to approve/reject/delete a Leave, Advance, or Overtime request that an **Admin** submitted — a regular Admin sees that request in the approval list with an "Awaiting CEO" badge instead of Approve/Reject/Delete buttons (requests from Employees, or from the CEO themself, work exactly as before — any Admin or the CEO can decide those). There's no in-app way to promote an existing account to CEO or to create the very first one — set `Role` to `ceo` directly in the `Users` sheet; after that, a CEO session can create further CEO accounts via "+ Add Employee" (a regular Admin's Add Employee form doesn't offer CEO, so one Admin can't mint another CEO account to work around the restriction) |
...
**Letters** — ID | UID | Type *(Warning / Note / Appreciation)* | Subject *(Warning/Appreciation only — blank on a Note)* | Message | Date | Issued By | Warning Number *(1/2/3 for a Warning letter — auto-computed and capped at 3 per employee; blank for Note/Appreciation)*
